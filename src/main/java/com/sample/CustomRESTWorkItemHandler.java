 ...
 * 
 * secured (that requires authentication) and open (no authentication).
 ...
 * overridden via work item parameters.
 * 
...
public class CustomRESTWorkItemHandler extends AbstractLogOrThrowWorkItemHandler implements Cacheable {
	...
	public CustomRESTWorkItemHandler(String username,
	...
	public CustomRESTWorkItemHandler(String handlingProcessId,
	...
	public CustomRESTWorkItemHandler(ClassLoader classLoader,
		...
		this(classLoader, doCacheClient, DEFAULT_TOTAL_POOL_CONNECTIONS, DEFAULT_MAX_POOL_CONNECTIONS_PER_ROUTE,
				handlingProcessId, handlingStrategy);
	...
	private void logProtocol() {
		...
		logger.debug(HTTP_CLIENT_API_43 ? "REST work item handler will use http client 4.3 api"
				: "REST work item handler will NOT use http client 4.3 api");
	...
	public void executeWorkItem(WorkItem workItem,
		...
		this.cookie = (String) workItem.getParameter(COOKIE);
		...
		try {
			...
			if (responseCode >= 200 && responseCode < 300) {
				...
				if (handleException) {
					...
					handleException(new CustomRESTServiceException(responseCode,
	...
	protected HttpResponse doRequestWithAuthorization(HttpClient httpclient,
		...
		if (type == null || type == AuthenticationType.NONE) {
			...
			try {
				...
				throw new RuntimeException(
						"Could not execute request [" + request.getMethod() + "] " + request.getURI(),
		...
		if (type == AuthenticationType.BASIC) {
			...
			try {
				...
				throw new RuntimeException(
						"Could not execute request with preemptive authentication [" + request.getMethod() + "] "
								+ request.getURI(),
	...
	protected HttpResponse doRequestWithAuthorization(HttpClient httpclient,
		...
		if (type == null || type == AuthenticationType.NONE) {
			...
			try {
				...
				throw new RuntimeException(
						"Could not execute request [" + httpMethod.getMethod() + "] " + httpMethod.getURI(),
...

	{
		...
		return proxyHost != null && proxyPort != null ? new DefaultProxyRoutePlanner(new HttpHost(proxyHost, proxyPort))
				: null;
	...
	protected Object configureRequest(String method,
		...
		if (HTTP_CLIENT_API_43) {
			...
			if (cookie != null) {
				theMethod.addHeader(COOKIE, cookie);
			}
